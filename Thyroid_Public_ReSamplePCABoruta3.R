...
for(i in seq(5, 17, by = 2)){
  for(j in seq(5, 12, by = 1)){
    ...
    filterName[k] = paste0(paste0(paste0("Filter ",i),", Bit "),j)
    k = k + 1
  }
}
...
png(paste0(paste0("Pub_Re_PCA_Bo_",fileName[f]),".png"),width = 1024, height = 699, units = "px", 
...
for(f in c(44,45,48,52,53,56)){
  ...
  new_mydata <- Boruta(Class_Label~., data = SMOTE_data, doTrace = 0)
  ...
  samples <- sample(NROW(Finaldata), NROW(Finaldata) * .632)
  data.train <- Finaldata[samples, ]
  data.test <- Finaldata[-samples, ]
