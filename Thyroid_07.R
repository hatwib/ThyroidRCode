...
for(f in 1:56){
  ...
  data.train.list <- lapply(data.train.list, function(d) getFeatures(d,FALSE))
  ...
  for(d in 1 : 4){
    tryCatch({
      ...
      boxplot(data$Values ~ data$Names, col=c('blue','red','green','yellow','pink','grey','orange') ,xaxt="n" , xlab = "", main = paste0(paste0(filterName[f]," with "),sampling.list[[d]]))
