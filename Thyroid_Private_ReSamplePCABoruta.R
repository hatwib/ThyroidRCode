...
#   PCA_row  <- PCA_row + 1
# 
# allData <- data.frame(prin_comp$x[,1:PCA_row])
...
png(paste0(paste0("Private_Re_FS_",f),".png"),width = 1024, height = 699, units = "px", 
...
# rm_idx <- length(rm_cols)
# 
# allData <- smotefamily::ADAS(allData[,-ncol(allData)],allData$Class_Label)$data
...
for(f in 3:56){
  ...
  #   PCA_row  <- PCA_row + 1
  # 
  # mydata <- data.frame(prin_comp$x[,1:PCA_row])
  # 
...
#plot(Boruta_SMOTE_data, xaxt="n")
#axis(1, at=1:35, labels=c(names(tValue)),  las = 2, hadj = 0.9)
#legend(x = "topright", legend = c("Training error", "10-fold CV error", "LOOCV error"),
#       lwd = rep(2, 3), col = c(gray(.4), "steelblue2", "darkorange"),
#       text.width = .3, cex = .85)
...
Final_data$Class_Label <- ifelse(Final_data$Class_Label == 1, 'Malignant', 'Benign')
Final_data$Class_Label <- as.factor(Final_data$Class_Label)
...
#data.model <- train(Class_Label ~., data.train, method = "rf", trControl = myTrainingControl, importance=TRUE, proximity=TRUE, nodesize = 1)
